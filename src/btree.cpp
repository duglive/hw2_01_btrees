...
//==============================================================================
    ...
    Byte* BaseBTree::search(const Byte* k)
    {
        ...
        while (true)
        {
            ...
            while (_comparator -> compare(page.getKey(i), k, _recSize) &&  i < key)
            {
                if (_comparator->isEqual(k, page.getKey(i), _recSize) &&  i < key)
                {
    ...
    int BaseBTree::PageWrapper::searchAll(const Byte* k, std::list<Byte*>& keys)
    {
        ...
        for(; i < keyNum && _tree -> _comparator -> compare(getKey(i), k, _tree -> _recSize); i++);

        PageWrapper page(_tree);
        if(!isLeaf())
        {
            page.readPageFromChild(*this, i);
            j += page.searchAll(k, keys);
        }

        while(i < keyNum && _tree->_comparator->isEqual(k, getKey(i), _tree->_recSize))
        {
...
//UInt BaseBTree::allocPageInternal(UShort keysNum, NodeType nt, PageWrapper& pw)
    ...
    {
        UInt pre = _rootPage.getPageNum(); // Previous root
        ...
        if (_rootPage.isFull())
        {
            ...
            _rootPage.setKeyNum(0);
            _rootPage.setCursor(0, pre);
...
//==============================================================================
    ...
    void BaseBTree::PageWrapper::splitChild(UShort iChild)
    {
        ...
        rightChild.allocPage((UShort) _tree -> _minKeys, leftChild.isLeaf());
        rightChild.copyKeys(rightChild.getKey(0), leftChild.getKey((UShort) (_tree -> _minKeys + 1)), (UShort) _tree -> _minKeys);

        if (!leftChild.isLeaf())
        {
            rightChild.copyCursors(rightChild.getCursorPtr(0), leftChild.getCursorPtr((UShort) (_tree -> _minKeys + 1)), (UShort) (_tree -> _minKeys + 1));
        }

        setKeyNum((UShort) (getKeysNum() + 1));

        UShort currentKey = getKeysNum();
        ...
        setCursor((UShort) (iChild + 1), rightChild.getPageNum());
        ...
        copyKey(getKey(iChild), leftChild.getKey((UShort) _tree -> _minKeys));
        leftChild.setKeyNum((UShort) _tree -> _minKeys);
    ...
    void BaseBTree::PageWrapper::insertNonFull(const Byte* k)
    {
        ...
        if (isLeaf())
        {
            ...
            while (c -> compare(k, getKey(i), _tree -> _recSize) && i >= 0)
            {
        ...
        if (child.getKeysNum() == 2 * _tree -> getOrder() - 1)
        {
            splitChild(i);
